...
template< class MapType > void print_map(const MapType & map,
...
void standardizeRules(LPI_Format& rules, uint32_t& atoms_count, LPI_Format& lpi_format_standardized) {
    ...
    std::for_each(rules.begin(), rules.end(), [](LPI_Rule& r){
        std::sort(r.begin() + LPI_IDX::RULE_BODY, r.end(), 
    ...
    // - introduce new atoms for same head rules that the body length is larger than 1
    ...
    while (idx != rules.end()) {
        ...
        if (count > 0) {    // if there are rules with the same head atom
            LPI_Rule new_or_rule = { LPI_RULE_TYPE::OR_RULE, (*idx)[LPI_IDX::RULE_HEAD] };
            for (auto it = idx; it != idx + count + 1; ++it) {
                if (it->size() > LPI_IDX::RULE_BODY + 1) { // body length is larger than 1
                    LPI_Rule new_and_rule = { LPI_RULE_TYPE::AND_RULE, (*idx)[LPI_IDX::RULE_HEAD] };
                    new_and_rule.insert(new_and_rule.end(), it->begin() + LPI_IDX::RULE_BODY, it->end());
                    
                    int head_atom;
                    auto body_it = (it + LPI_IDX::RULE_BODY);
                    body_rule_bimap::left_iterator found_it = duplicate_body_mapping.left.find(*body_it);
...
void printLPBitMatrix(BitMatrix mat, BitVector ruletype, uint32_t atoms_count) {
    for (auto it = mat.begin(); it != mat.end(); ++it) {
        std::cout << *it << "\t Type: " << (ruletype[it - mat.begin()] ? " OR" : "AND") << "\t";
...
int main(int argc, char const **argv) {
    ...
    // cgrounder.input_files({"./samples/model_graphcoloring.lp", "./samples/graph_6nodes.lp"});
    ...
    for (auto rule : rules_standardized) {
        ...
        // std::cout << "bit " << lp_matrix[rule[LPI_IDX::RULE_HEAD] - 1] << std::endl;
    }
